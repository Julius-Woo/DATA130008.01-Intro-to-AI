...
class MazeRLAgent(object):
    ...
    def policy_evaluation(self, env, values, policy):
        while True:
            new_values = {s:0.0 for s in env.legal_states}
            difference = .0
            # TODO: begin your code
            for s in env.legal_states:
                v = values[s]
                a = policy[s]
                ...
                if done:
                    ...
                    new_values[s] = reward + prob * self.gamma * values[next_state]
                difference += abs(v - new_values[s])
            # ------ end ----------
    ...
    def policy_iteration(self, env):
        """ POLICY iteration. """
        values = {s:0.0 for s in env.legal_states}
        policy = {s:random.choice(env.ACTIONS) for s in env.legal_states}
    ...
    def value_iteration(self, env):
        """ VALUE iteration. """
        values = {s:0.0 for s in env.legal_states}
        policy = {s:None for s in env.legal_states}
        
        self.iteration_number = 0
        while True:
            difference = .0
            # TODO: begin your code
            new_values = {s: 0.0 for s in env.legal_states}  # Synchronous Update
            new_policy = {s:None for s in env.legal_states}
            self.iteration_number += 1
            for s in env.legal_states:
                action_values = {}
                for a in env.legal_actions(s):
                    ...
                    if done:
                        ...
                        action_values[a] = reward + prob * self.gamma * values[next_state]
                a_max = max(action_values, key=action_values.get)
                v_max = action_values[a_max]
                difference += abs(values[s] - v_max)
                new_policy[s] = a_max
                new_values[s] = v_max
            # ------ end ----------
