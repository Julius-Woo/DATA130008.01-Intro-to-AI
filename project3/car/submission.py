...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                dis = math.sqrt((x - agentX) ** 2 + (y - agentY) ** 2)
                self.belief.grid[row][col] *= util.pdf(dis, Const.SONAR_STD, observedDist)
    ...
    def elapseTime(self):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                oldBelief[(row, col)] = self.belief.getProb(row, col)
                self.belief.setProb(row, col, 0)  # reset belief to 0
        for k, v in self.transProb.items():
            oldTile, newTile = k
            self.belief.addProb(newTile[0], newTile[1], delta=oldBelief[oldTile] * v)
        self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for r,c in self.particles:
            ...
            dis = math.sqrt((x - agentX) ** 2 + (y - agentY) ** 2)
            new_weights[(r,c)] = util.pdf(dis, Const.SONAR_STD, observedDist) * self.particles[(r,c)]  # re-weighting
        self.particles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            new_particle = util.weightedRandomChoice(new_weights)  # re-sampling
